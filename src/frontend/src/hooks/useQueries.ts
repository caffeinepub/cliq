...
import type { UserProfile, Post, Comment, MarketplaceListing, Conversation, Message, Notification, TicketEvent, Ticket, MediaAttachment } from '../backend';
...
export function useCreatePost() {
  ...
  return useMutation({
    mutationFn: async ({ content, media }: { content: string; media?: MediaAttachment | null }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useGetFollowingFeed(limit: number = 20, offset: number = 0) {
  const { actor, isFetching: actorFetching } = useActor();
...
export function useGetCampusFeed(limit: number = 20, offset: number = 0) {
  const { actor, isFetching: actorFetching } = useActor();
...
export function useGetUniversalFeed(limit: number = 20, offset: number = 0) {
  const { actor, isFetching: actorFetching } = useActor();
...
export function useAddComment() {
  ...
  return useMutation({
    mutationFn: async ({ postId, content, parentComment }: { postId: bigint; content: string; parentComment?: bigint | null }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useCreateListing() {
  ...
  return useMutation({
    mutationFn: async ({ title, description, price, media }: { title: string; description: string; price: number; media?: ExternalBlob | null }) => {
      if (!actor) throw new Error('Actor not available');
      return await actor.createListing(title, description, BigInt(price), media || null);
    },
    onSuccess: () => {
...
export function useGetListing(listingId?: string) {
  ...
  return useQuery<MarketplaceListing | null>({
    ...
    queryFn: async () => {
      if (!actor || !listingId) throw new Error('Actor or listing ID not available');
...
export function useSearchListings(searchTerm?: string, universityFilter?: string) {
  const { actor, isFetching: actorFetching } = useActor();
  ...
  return useQuery<MarketplaceListing[]>({
    ...
    queryFn: async () => {
      if (!actor) throw new Error('Actor not available');
      return await actor.searchListings(searchTerm || null, universityFilter || null);
    },
    enabled: !!actor && !actorFetching,
  });
}
...
export function useCreateTicketEvent() {
  ...
  return useMutation({
    mutationFn: async ({ title, description, eventDate, verifiers, ticketPrice, availableTickets }: { title: string; description: string; eventDate: bigint; verifiers: string[]; ticketPrice?: number; availableTickets?: number }) => {
      if (!actor) throw new Error('Actor not available');
      const { Principal } = await import('@dfinity/principal');
...
export function useGetTicketEvent(eventId?: string) {
  ...
  return useQuery<TicketEvent | null>({
    ...
    queryFn: async () => {
      if (!actor || !eventId) throw new Error('Actor or event ID not available');
...
export function useBrowseTicketEvents(universityFilter?: string | null, limit: number = 20, offset: number = 0) {
  const { actor, isFetching: actorFetching } = useActor();
  ...
  return useQuery<TicketEvent[]>({
    ...
    queryFn: async () => {
      if (!actor) throw new Error('Actor not available');
      return await actor.browseTicketEvents(universityFilter || null, BigInt(limit), BigInt(offset));
    },
    enabled: !!actor && !actorFetching,
  });
}
...
export function useIssueTicket() {
  ...
  return useMutation({
    mutationFn: async ({ eventId, attendee, code }: { eventId: bigint; attendee: string; code: string }) => {
      if (!actor) throw new Error('Actor not available');
      const { Principal } = await import('@dfinity/principal');
    ...
    onSuccess: (_, { eventId }) => {
      queryClient.invalidateQueries({ queryKey: ['eventTickets', eventId.toString()] });
    },
  });
}
