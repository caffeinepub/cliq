...
import { useGetConversations, useStartConversation, useSearchUsers } from '../hooks/useQueries';
...
function ConversationItem({ conversation }: { conversation: Conversation }) {
  ...
  return (
    <Card
      ...
      onClick={() => navigate({ to: '/messages/$conversationId', params: { conversationId: conversation.id.toString() } })}
    >
      <CardContent className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-semibold">{otherUser?.displayName || 'Unknown'}</div>
            <div className="text-sm text-muted-foreground">@{otherUser?.username || 'unknown'}</div>
          </div>
        </div>
      </CardContent>
    </Card>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {searchResults && searchResults.length > 0 ? (
                  ...
                  <p className="text-center text-muted-foreground py-4">Search for a user to start a conversation</p>
                )}
              </div>
            </div>
          </DialogContent>
