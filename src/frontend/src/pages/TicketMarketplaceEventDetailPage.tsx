...
import { useGetTicketEvent, usePurchaseTicket, useGetMyTickets } from '../hooks/useQueries';
import { Loader2, ArrowLeft, Calendar, MapPin, Ticket as TicketIcon, Users } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { toast } from 'sonner';

export function TicketMarketplaceEventDetailPage() {
  const { eventId } = useParams({ from: '/marketplace/tickets/$eventId' });
  ...
  const hasTicket = myTickets?.some((ticket) => ticket.eventId.toString() === eventId);
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      toast.success('Ticket purchased successfully! Check "My Tickets" to view it.');
    } catch (error: any) {
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="border-b pb-4 flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate({ to: '/marketplace/tickets' })}>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{event.university}</div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {Number(event.availableTickets)} {Number(event.availableTickets) === 1 ? 'ticket' : 'tickets'} remaining
                </div>
              </div>
            </div>
            ...
            {event.verifiers.length > 0 && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {event.verifiers.length} authorized {event.verifiers.length === 1 ? 'verifier' : 'verifiers'}
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="border-t pt-4">
            {hasTicket ? (
              <div className="bg-accent/50 rounded-lg p-4 text-center">
                ...
                <p className="font-medium">You already have a ticket for this event</p>
                <p className="text-sm text-muted-foreground mt-1">Check "My Tickets" to view it</p>
                <Button
                  variant="outline"
                  className="mt-3"
              ...
              <div className="bg-destructive/10 rounded-lg p-4 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">This event has no more tickets available</p>
              </div>
            ) : (
              <Button
                className="w-full"
